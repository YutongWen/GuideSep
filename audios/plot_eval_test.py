...
root_dir = '/Users/cooper/Desktop/separation_examples/demo/audios'
ins_list = ['piano', 'guitar', 'strings', 'bass', 'brass', 'synth', 'pipe', 'reed', 'organ', 'cp']
row_list = ['mixture', 'cond', 'melodysep',  'baseline', 'clean_ref']
raw_content = []
sample_rate = 16000
...
for i, row in enumerate(row_list):
    audios = []
    for ins in ins_list:
        filename = os.path.join(root_dir, f'*{ins}', f'*{row}*.wav')
...
row_header = ['Target Instrument', 'Mixture', 'Melody Condition', 'MelodySep Result',  'Mask-prediction Baseline', 'Ground-truth']
c_header = ['Piano', 'Guitar', 'Strings', 'Bass', 'Brass', 'Synth', 'Pipe', 'Reed', 'Organ', 'Chromatic Percussion']
...
sg = soundgrid(*raw_content, sr=sample_rate, r_header=row_header, c_header=c_header, show='mel')
