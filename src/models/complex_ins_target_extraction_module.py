...
class ConditionInsExtractionComplex(LightningModule):
    ...
    def __init__(
        ...
        inference_cond_gen: bool = False,
        inference_inpaint: bool = False,
        inference_uncondition: bool = False,
        use_psuedo_masks: bool = False,
    ):
        super().__init__()
        ...
        self.condition_mix_drop_prob = condition_mix_drop_prob
        ...
        self.use_res_channel = use_res_channel
        self.n_fft = n_fft
        ...
        self.inference_cond_gen = inference_cond_gen
        self.inference_inpaint = inference_inpaint
        self.inference_uncondition = inference_uncondition
    ...
    def forward(self, clean, condition, mixture, residual):
        ...
        spec = spec_fwd(spec, spec_abs_exponent=self.spec_abs_exponent, 
                        spec_factor=self.spec_factor).unsqueeze(1)
        spec_mix = spec_fwd(spec_mix, spec_abs_exponent=self.spec_abs_exponent, 
                             spec_factor=self.spec_factor).unsqueeze(1)
        spec_cond = spec_fwd(spec_cond, spec_abs_exponent=self.spec_abs_exponent, 
                             spec_factor=self.spec_factor).unsqueeze(1)
        
        spec = torch.cat((spec.real, spec.imag), dim=1)
        spec_mix = torch.cat((spec_mix.real, spec_mix.imag), dim=1)
        # spec_cond = torch.cat((spec_cond.real, spec_cond.imag), dim=1)
        spec_cond = spec_cond.abs()
        
        if self.use_res_channel:
            spec_res = torch.stft(residual, window=self.window.to(self.device), 
                                    normalized=True, return_complex=True, 
                                    **self.stft_args)
            spec_res = spec_fwd(spec_res, spec_abs_exponent=self.spec_abs_exponent, 
                                spec_factor=self.spec_factor).unsqueeze(1)        
            spec_res = torch.cat((spec_res.real, spec_res.imag), dim=1)    
            spec = torch.cat((spec, spec_res), dim = 1)
        ...
        mix_mask = drop_mask(drop_prob=self.condition_mix_drop_prob, batch_size=batch_size, device=self.device)
        
        # make sure mix is not the only condition and at least one condition is provided
        _cond_mask = _no_mel_mask & (~mix_mask) 
        no_condition = _cond_mask & _no_mel_mask & mix_mask
        _cond_mask = _cond_mask | no_condition
        condition_mask = torch.where(_cond_mask, False, condition_mask)
        
        spec_cond = torch.where(condition_mask, torch.tensor(-1.0), spec_cond)
        spec_mix = torch.where(mix_mask, torch.tensor(-1.0), spec_mix)
        
        # spec_cond = torch.where(, torch.tensor(0.0), spec_cond)
        # make sure mix is not the only condition
        
        
        
        # spec.shape (batch, 2, 256, 256)
        # (batch, 256, 128)
        condition = torch.cat((spec_mix, spec_cond, mel_mask), dim=1) # (batch, 5, 256, 256)
        # condition = condition * uncondition_masks
        
        # Sample amount of noise to add for each batch element
        sigmas = self.noise_distribution(num_samples=spec.shape[0], device=spec.device)
        
        # compute loss
        loss = self.diffusion(spec, self.net, 
                              sigmas=sigmas,
    ...
    def on_validation_epoch_end(self):
        ...
        for i, val_instance in enumerate(self.val_instances):
            ...
            with torch.no_grad():
                audio_sample, _ = self.inference(condition=(cond_signal, mel_p_mask, mel_n_mask), 
                                                    mixture=mixture, 
                                                    diff_net=diff_net,
                                                    p_mask=p_mask,
                                                    n_mask=n_mask)
    ...
    def inference(self, condition, 
        ...
        with torch.no_grad():
            ...
            if condition is None:
                time_length = spec_mixture.shape[-1]//16*16 # make sure the length is even
                condition = -1.0 * torch.ones((mixture.shape[0], 1, self.n_fft//2+1, time_length), device=self.device)
            else:
                condition = torch.stft(condition, window=self.window.to(self.device), 
                                normalized=True, return_complex=True, 
                                **self.stft_args)
                
                time_length = min(spec_mixture.shape[-1], condition.shape[-1])//16*16
                
                condition = condition[..., :time_length]
                
                condition = spec_fwd(condition, spec_abs_exponent=self.spec_abs_exponent, 
                                    spec_factor=self.spec_factor).unsqueeze(1)
                condition = condition.abs()

            spec_mixture = spec_mixture[..., :time_length]
            
            spec_mixture = spec_fwd(spec_mixture, spec_abs_exponent=self.spec_abs_exponent, 
                                spec_factor=self.spec_factor).unsqueeze(1)
            spec_mixture = torch.cat((spec_mixture.real, spec_mixture.imag), dim=1)
            
            # pad or trim mel mask to time_length
            if mel_p_mask.shape[-1] < time_length:
                mel_p_mask = torch.cat((mel_p_mask, torch.zeros((mel_p_mask.shape[0], mel_p_mask.shape[1], time_length - mel_p_mask.shape[-1]), device=mel_p_mask.device)), dim=-1)
            elif mel_p_mask.shape[-1] > time_length:
                mel_p_mask = mel_p_mask[..., :time_length]
                
            if mel_n_mask.shape[-1] > time_length:
                mel_n_mask = mel_n_mask[..., :time_length]
            elif mel_n_mask.shape[-1] < time_length:
                mel_n_mask = torch.cat((mel_n_mask, torch.zeros((mel_n_mask.shape[0], mel_n_mask.shape[1], time_length - mel_n_mask.shape[-1]), device=mel_n_mask.device)), dim=-1)
            ...
            # initial noise
            channel = 4 if self.use_res_channel else 2
            X_noise = torch.randn((condition.shape[0], channel, self.n_fft//2+1, time_length), device=self.device)
            
            # synthesize "complex" spec
            pcomplex_spec = self.sampler(X_noise, fn=self.diffusion.denoise_fn, 
                                        net=diff_net, sigmas=self.noise_scheduler.to(self.device), 
            ...
            if self.use_res_channel:
                pcomplex_spec, residual = torch.split(pcomplex_spec, 2, dim=-1)
                res_complex_spec = torch.view_as_complex(residual)[:, None, :, :]
                res_complex_spec = spec_back(res_complex_spec,
                                            spec_abs_exponent=self.spec_abs_exponent, 
                                            spec_factor=self.spec_factor)
                residual = torch.istft(res_complex_spec.squeeze(1), 
            ...
            target_complex_spec = torch.view_as_complex(pcomplex_spec)[:, None, :, :]
            target_complex_spec = spec_back(target_complex_spec,
                                        spec_abs_exponent=self.spec_abs_exponent, 
                                        spec_factor=self.spec_factor)
            ...
            # convert to waveform
            target = torch.istft(target_complex_spec.squeeze(1), 
                                        window=self.window.to(self.device), 
                                        normalized=True,
                                        **self.stft_args)
            target = target.cpu()
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        if clean is None or clean.eq(0).all():
        ...
        with torch.no_grad():
            # audio_sample = self.inference(midi=torch.cat((midi, midi_others), dim=0), 
        ...
        clean = clean[..., :audio_sample.shape[-1]]
        mixture = mixture[..., :audio_sample.shape[-1]]
        ...
        if batch_idx % 1 == 0:
            ...
            self.plot_mel_mask(mel_spec_mask_tar=mel_p_mask,
                            mel_spec_mask_res=mel_n_mask,
                            mel_spec_tar=mel_spec_mask_cond,
                            mel_spec_res=mel_spec_mask_mix,
