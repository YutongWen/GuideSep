...
class UpsamplingBlock(nn.Module):
    ...
    def forward(self, x, t, c, context, context_mask, down_hiddens, up_hiddens, inj_channels=None):
        if inj_channels is not None:
            x = x + inj_channels
...
class UNet2dBase(nn.Module):
    ...
    def forward(self, 
        ...
        if inj_channels is not None and self.use_condition_block:
            # if cond_drop_prob == 1:
            #     mask = -1.0 * torch.ones(inj_channels.shape[0], 2, inj_channels.shape[2], inj_channels.shape[3]).to(inj_channels.device)
            #     inj_channels_last = inj_channels[:, 2:, :, :]
            #     inj_channels = torch.cat((mask, inj_channels_last), dim=1)
            
            # inj_keep_mask = torch.all(inj_channels != -1.0, dim=(1, 2, 3))
            # print(inj_keep_mask)
            # exit()
        ...
        if self.num_classes != 0:
            ...
            classese_cond_drop_prob = cond_drop_prob
            if classes is None:
                classese_cond_drop_prob = 1.0
                classes = torch.tensor([0]).to(t.device)
            classes_emb = self.label_conditioner(classes, classese_cond_drop_prob)
            t = t + classes_emb
        ...
        if exists(self.init_resnet_block):
            ...
            if inj_channels is not None and self.use_condition_block:
                inj_channels = self.init_resnet_block_cond(inj_channels, t)
            if mixture is not None and self.mixture_input:
                mixture = self.init_resnet_block_mix(mixture, t)
        ...
        if self.use_condition_block and inj_channels is not None:
            ...
            for cond_block, block in zip(self.downs_cond, self.downs):
                ...
                x, hiddens = block(x, t=t, c=None, 
                                   ...
                                   hiddens=hiddens, inj_channels=condition)
                inj_channels, cond_hiddens = cond_block(inj_channels, t=t, c=None,
                                             context_mask=context_mask, 
        ...
        up_hiddens = []
        if self.use_condition_block and inj_channels is not None:
            inj_channels = self.cond_mid_block(inj_channels, t, c=None)
            cond_up_hiddens = []
            ...
            for cond_block, block in zip(self.ups_cond, self.ups):
                ...
                inj_channels, cond_hiddens, cond_up_hiddens = cond_block(inj_channels, t, c=None,
                                                                         context=context, 
                                                                         context_mask=context_mask, 
...
if __name__ == '__main__':
    ...
    input_tensor = torch.zeros(8, 4, 256, 256)
    cond_tensor = torch.zeros(8, 5, 256, 256)
    sigmas = torch.zeros(8,)
    model = UNet2dBase(cond_on_text=True,
                        text_embed_dim=768,
                        max_text_len=120,
                        dim=256,
                        cond_drop_prob=0.0,
